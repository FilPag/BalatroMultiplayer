...
return {
	...
	misc = {
		...
		dictionary = {
			...
			k_warning_unlock_profile = "現在使用しているプロフィールは、全コレクションがアンロックされていません。ランク戦/トーナメント戦の場合は、新しいプロフィールを作成し、プロフィール設定ですべてのアンロックを解除してください。",
			k_warning_cheating = "これが表示される場合、対戦相手が不正を行っている可能性があります。ランク戦であれば、Discordサーバーにて「%s」というメッセージを送り、supportチャンネルでサポートチケットを開いてください。",
